...
-- Review trades: backtest trades generated by review.py (not persisted via run_id).
-- Written so that trade_viewer can look up any trade by ID without the user
-- having to manually enter symbol/date.
CREATE TABLE IF NOT EXISTS review_trades (
    ...
    r_multiple  DOUBLE,
    planned_rr  DOUBLE,
    config_json JSON,               -- serialised BacktestParams (for entry_tf lookup)
    created_at  TIMESTAMP DEFAULT now()
);
...
class BacktestDB:
        ...
        self._conn.executemany(
            ...
            INSERT INTO review_trades
                (trade_id, symbol, direction, entry_time, exit_time,
                 ...
                 result, r_multiple, planned_rr, config_json)
            VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)
    ...
    def fetch_review_trade(self, trade_id: str) -> Optional[dict]:
        row = self._conn.execute(
